...
public:
	...
	trajectory_msgs::JointTrajectory getPreGraspPosture() {
		...
		t.joint_names.push_back("right_sdh_finger_21_joint");
		...
		pt.positions.resize(8);
		pt.positions[0] = 0.0;
		pt.positions[1] = -M_PI / 4;	// -30 degrees
		pt.positions[2] = M_PI / 9;	// +20 degrees
		pt.positions[3] = 0.0;
		pt.positions[4] = -M_PI / 4;	// -30 degrees
		pt.positions[5] = M_PI / 9;	// +20 degrees
		pt.positions[6] = -M_PI / 4;	// -30 degrees
		pt.positions[7] = M_PI / 9;	// +20 degrees
	...
	trajectory_msgs::JointTrajectory getGraspPosture() {
		...
		pt.positions.resize(8);
		pt.positions[0] = 0.0;
		pt.positions[1] = -M_PI / 14;
		pt.positions[2] = M_PI / 6;	// +20 degrees
		pt.positions[3] = 0.0;
		pt.positions[4] = -M_PI / 14;	// -30 degrees
		pt.positions[5] = M_PI / 6;	// +20 degrees
		pt.positions[6] = -M_PI / 14;	// -30 degrees
		pt.positions[7] = M_PI / 6;	// +20 degrees
	...
	bool place(const geometry_msgs::Pose& goal_pose, std::string name) {
		...
		for (double angle = 0; angle < 2 * M_PI; angle += M_PI / 4) {
			...
			Eigen::Quaterniond quat(
					Eigen::AngleAxis<double>(double(angle),
...
int main(int argc, char **argv) {
	...
	uibk_pick_place::PickPlace t(nh);
	t.start();
