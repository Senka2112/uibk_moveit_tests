...
PlanExecution::PlanExecution(const string &name) {
...
PlanningResultPtr PlanExecution::plan_pick(const string &object_id, const std::vector<Grasp> &grasps) {

	PlanningResultPtr result;
	result.reset(new PlanningResult);
	result->type = PlanExecution::PICK;

	if (!pick_action_client_) {
		ROS_ERROR("Pick action client not found");

		result->status = PlanExecution::FAILURE;
	...
	if (pick_action_client_->getState()	== actionlib::SimpleClientGoalState::SUCCEEDED) {
		...
		result->status = PlanExecution::SUCCESS;
...
PlanningResultPtr PlanExecution::plan_place(const string &object_id, const vector<PlaceLocation> &locations) {

	PlanningResultPtr result;
	result.reset(new PlanningResult);
	result->type = PlanExecution::PLACE;
...
PlanningResultPtr PlanExecution::plan(const geometry_msgs::Pose &pose_goal) {

	PlanningResultPtr result;
	result.reset(new PlanningResult);
	result->type = PlanExecution::GOAL;
...
bool PlanExecution::execute(const PlanningResultPtr& plan) {
	for (size_t i = 0; i < plan->trajectory_stages.size(); ++i) {
		...
		if(description == "grasp" && plan->type == PlanExecution::PICK) {
			attachObject(plan->object_id);
		}
		else if(description == "grasp" && plan->type == PlanExecution::PLACE) {
...
bool PlanExecution::execute(const RobotTrajectory &trajectory) {
...
bool PlanExecution::attachObject(const string &object) {
...
void PlanExecution::setPlanningAttempts(int value) {
	planning_attempts_ = value;
}

int PlanExecution::getPlanningAttempts() {
	return planning_attempts_;
}

bool PlanExecution::detachObject(const string &object) {
	moveit_msgs::AttachedCollisionObject aco;
...
void PlanExecution::setPlanningGroupName(const string& name) {
	if (!robot_model_->hasJointModelGroup(name)) {
...
string PlanExecution::getPlanningGroupName() {
	return group_name_;
}

void PlanExecution::setPlannerId(const string& planner_id) {
	planner_id_ = planner_id;
}

string PlanExecution::getPlannerId() {
	return planner_id_;
}

void PlanExecution::setAllowedPlanningTime(double value) {
	planning_time_ = value;
}

double PlanExecution::getAllowedPlanningTime() {
	return planning_time_;
}

void PlanExecution::setSupportSurfaceName(const string name) {
	support_surface_ = name;
}

string PlanExecution::getSupportSurfaceName() {
